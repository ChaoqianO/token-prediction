...
try:
    ...
    from scripts.verify_data_foundation_baseline import _assert_privacy_safe
...
def _validate_lock(value: Any) -> Mapping[str, Any]:
    ...
    _text(artifact["baseline_id"], label="baseline id")
    for key in (
        "artifact_id",
        "manifest_sha256",
        "results_file_sha256",
    ...
    _count(protocol["fold_count"], label="prediction lock fold count")
    _number(protocol["alpha"], label="prediction lock alpha")
    if (
    ...
    if holdout["independent_of_split_seed_labels_and_suffixes"] is not True:
        raise PredictionLockError("prediction lock holdout independence is not true")
