...
class FrozenSourceDescriptorTests(unittest.TestCase):
    def test_tracked_descriptors_match_reader_capability_contracts(self) -> None:
        cases = {
            "bagen_swebench.json": BagenSwebenchReader.capabilities,
            "spend_openhands.json": OpenHandsArchiveReader.capabilities,
        }
        for filename, reader_capabilities in cases.items():
