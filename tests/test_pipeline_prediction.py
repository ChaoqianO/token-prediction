...
from token_prediction.experiment import CandidateRole, CandidateSpec
...
from token_prediction.pipeline import load_trajectory, run_configured_experiments
...
class PredictionPipelineSmokeTests(unittest.TestCase):
    ...
    def test_config_drives_real_fit_predict_evaluate_artifact(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            config = _v2_config(
                load_config(ROOT / "configs" / "mvp.toml"), root, event_paths
            )
            ...
            self.assertEqual(summary.candidate_run_count, 6)
            ...
            for candidates in metrics.values():
                for candidate in candidates.values():
                    self.assertEqual(set(candidate["fold_metrics"]), {"0", "1", "2", "3", "4"})
            with patch(
    ...
    def test_source_change_during_training_refuses_artifact_publication(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch(
                ...
                side_effect=("before-training", "after-training"),
            ):
    ...
    def test_lightgbm_fold_audit_artifacts_and_runtime_versions_are_published(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            metrics = json.loads((output / "metrics.json").read_text(encoding="utf-8"))
            self.assertEqual(
                set(metrics["lightgbm-audit"]["lightgbm"]["fold_metrics"]),
                {"0", "1", "2", "3", "4"},
            )
            ...
            for fold in range(5):
                ...
                fit_report = json.loads(
                    (fold_dir / "fit_report.json").read_text(encoding="utf-8")
                )
                self.assertEqual(
                ...
                expected = next(
                    ...
                    if row["candidate_id"] == "lightgbm" and row["fold"] == fold
                )
                raw_point = point_by_id[expected["point_id"]]
                selected = raw_point.with_features(
                    feature_set.select(raw_point.features)
                )
    ...
    def test_unsafe_experiment_id_is_rejected_before_training(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            malicious = replace(
                base_config.experiments[0], experiment_id="../outside"
            )
            config = replace(
                base_config,
